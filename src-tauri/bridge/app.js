...
function parseModelKey(val) {
  const i = String(val || '').indexOf(':');
  if (i < 0) return { provider: String(val || ''), model: '' };
  return { provider: val.slice(0, i), model: val.slice(i + 1) };
}
...
function safeMediaUrl(url) {
  ...
  if (/^data:(video|image)\//i.test(u)) return u;
...
async function requestChat(provider, model, messages) {
...
function appendMediaMessage(kind, url) {
  ...
  if (!safe) {
    addMessage('Güvenli olmayan medya URL reddedildi.', 'bot');
    return null;
  }
...
const PROVIDERS = {
  ...
  nvidia: {
    ...
    why: 'NVIDIA\'nın NIM inference platformu. Tüm modeller prototype için ücretsiz (40 RPM, kredi bazlı). Nemotron, DeepSeek, GLM, Kimi, Mistral ve 50+ model.',
    url: 'https://build.nvidia.com/',
    urlText: 'build.nvidia.com',
    keyFormat: 'nvapi-...',
    models: '80+ model: Nemotron 3 Ultra/Super, DeepSeek V4.1, GLM 5.3, Kimi K3, GPT-OSS, Mistral, Gemma 4...',
    pricing: 'Tüm modeller ücretsiz (prototype, 40 RPM)',
    limit: '40 istek/dk, 1000-5000 kredi'
  },
  airforce: {
...
async function send() {
  ...
  try {
    if (provider === 'image') {
      ...
      const url = 'https://image.pollinations.ai/prompt/' + encoded + '?model=' + encodeURIComponent(model) + '&width=1024&height=1024';
      typing.remove();
      appendMediaMessage('img', url);
      pushHistory('user', text);
      pushHistory('assistant', 'Görsel üretildi: ' + text);
      ...
      if (d.video) {
        notifySound();
        appendMediaMessage('video', d.video);
        pushHistory('user', text);
        pushHistory('assistant', 'Video üretildi: ' + text);
      } else if (d.job_id) {
        ...
        const poll = async () => {
          ...
          if (attempts > 60) {
            errorSound();
            const c = statusMsg.querySelector('.msg-content');
            if (c) c.textContent = 'Video çok uzun sürdü. Kontrol edin.';
            return;
          }
          try {
            const pr = await fetch(API + '/video-status?job_id=' + encodeURIComponent(d.job_id));
            const pd = await pr.json();
            if (pd.status === 'completed' && pd.video_url) {
              notifySound();
              statusMsg.innerHTML = '';
              const vid = document.createElement('video');
              const safe = safeMediaUrl(pd.video_url);
              if (safe) {
                vid.src = safe;
                vid.controls = true;
                vid.autoplay = true;
                ...
                statusMsg.appendChild(vid);
              } else {
                const c = statusMsg.querySelector('.msg-content');
                if (c) c.textContent = 'Güvenli olmayan video URL.';
              }
            } else if (pd.status === 'failed') {
              errorSound();
              const c = statusMsg.querySelector('.msg-content');
              if (c) c.textContent = 'Hata: ' + (pd.error || 'Başarısız');
            } else {
...
function initDropdown() {
  ...
  groupOrder.forEach(provider => {
    ...
    models.forEach(m => {
      ...
      html += `<div class="dropdown-option${isActive}" data-key="${m.key}" data-provider="${provider}" role="option" tabindex="0">
...
if (dropdownList) {
  ...
  dropdownList.addEventListener('change', (e) => {
    if (e.target?.type === 'checkbox') filterModels();
  });
}
...
window.selectModel = function(id) {
  ...
  const activeOpt = Array.from(document.querySelectorAll('.dropdown-option'))
    .find(o => o.getAttribute('data-key') === id);
...
if (dropdownList) {
  dropdownList.addEventListener('mouseover', (e) => {
    const opt = e.target.closest?.('.dropdown-option');
    if (!opt || !dropdownList.contains(opt)) return;
    const md = MODEL_DB[opt.getAttribute('data-key')];
    if (md) showTooltip(md, e);
  });
  dropdownList.addEventListener('mousemove', (e) => {
    if (e.target.closest?.('.dropdown-option')) moveTooltip(e);
  });
  dropdownList.addEventListener('mouseout', (e) => {
    const from = e.target.closest?.('.dropdown-option');
    const to = e.relatedTarget?.closest?.('.dropdown-option');
...
window.filterModels = function(input) {
  ...
  // Panel filtrelerini al (checkbox)
  const checkedIn = (containerId) => {
    const box = document.getElementById(containerId);
    if (!box) return [];
    return Array.from(box.querySelectorAll('input[type="checkbox"]:checked')).map(i => i.value);
  };
  const panelProvider = checkedIn('filterProviderOptions');
  const panelPrice = checkedIn('filterPriceOptions');
  const panelRating = checkedIn('filterRatingOptions');
  const panelCap = checkedIn('filterCapabilityOptions');
  ...
  options.forEach(opt => {
    ...
    // Sağlayıcı filtresi (arama kutusu @ ile panel checkbox birlikte AND)
    const searchProviderOk = !searchProviderFilter || providerName.includes(searchProviderFilter);
    const panelProviderOk = panelProvider.length === 0 ||
      panelProvider.some(pv => {
        const needle = pv.toLowerCase();
        return providerName === needle || providerName.includes(needle);
      });
    const providerOk = searchProviderOk && panelProviderOk;
...
function populateFilterOptions() {
  ...
  const providerBox = document.getElementById('filterProviderOptions');
  if (providerBox && !providerBox.dataset.populated) {
    providers.forEach(prov => {
      ...
      cb.type = 'checkbox';
      cb.value = prov;
      cb.onchange = () => filterModels();
      label.appendChild(cb);
      label.appendChild(document.createTextNode(' ' + prov));
      providerBox.appendChild(label);
    });
    providerBox.dataset.populated = 'true';
  }

  const allCaps = [...new Set(Object.values(MODEL_DB).flatMap(m => m.caps || []))].sort((a, b) => a.localeCompare(b, 'tr'));
  const capBox = document.getElementById('filterCapabilityOptions');
  if (capBox && !capBox.dataset.populated) {
    allCaps.forEach(cap => {
      const label = document.createElement('label');
      label.className = 'filter-check';
      const cb = document.createElement('input');
      ...
      capBox.appendChild(label);
    });
    capBox.dataset.populated = 'true';
  }
}
...
window.clearAllFilters = function() {
  document.querySelectorAll('#filterDropdown input[type="checkbox"]:checked').forEach(cb => { cb.checked = false; });
...
// İzin ver (gerçek mikrofon izni)
window.grantMicPermission = async function() {
...
function ensureListening() {
  if (!isVoiceActive || isSpeaking) return;
  if (recognition && isListening) return;
  startListening();
}

// Dinleme başlat (Web Speech API - Chrome/Edge)
function startListening() {
  if (!isVoiceActive || isSpeaking) return;
  if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    startWebSpeechRecognition();
  } else {
    updateVoiceStatus('❌ Tarayıcınız sesli tanımayı desteklemiyor. Chrome/Edge kullanın.', 'error');
  }
}
...
function startWebSpeechRecognition() {
  ...
  recognition.onerror = function(event) {
    if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
      ...
      isVoiceActive = false;
      updateVoiceButton();
    } else if (event.error !== 'aborted' && event.error !== 'no-speech' && isVoiceActive) {
  ...
  recognition.onend = function() {
    ...
    if (isVoiceActive && !isSpeaking) {
      setTimeout(() => {
        if (isVoiceActive && !isSpeaking) {
          try {
            if (!recognition) startWebSpeechRecognition();
            else recognition.start();
            isListening = true;
            updateVoiceButton();
          } catch(e) {
            recognition = null;
            setTimeout(() => { if (isVoiceActive && !isSpeaking) startListening(); }, 400);
          }
        }
      }, 250);
    }
